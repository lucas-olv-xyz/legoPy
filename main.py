...
class BatchSwitcherApp(tk.Tk):
    ...
    def show_batch_menu(self):
        self.clear_main()
        label = ttk.Label(self.main_frame, text="Choose Batch", font=("Arial", 22, "bold"), style='AppHeading.TLabel')
        label.pack(pady=(40, 10))

        proj_frame = ttk.Frame(self.main_frame, style='App.TFrame')
        proj_frame.pack(pady=(12, 20))
        ttk.Label(proj_frame, text="Project Code:", font=("Arial", 13), style='App.TLabel').pack(side="left", padx=(0,6))
        ...
        btn_first = ttk.Button(self.main_frame, text="First Batch", width=25, command=self.show_first_batch, style='Accent.TButton')
        btn_first.pack(pady=8)
        btn_next = ttk.Button(self.main_frame, text="Next Batch", width=25, command=self.show_next_batch, style='Accent.TButton')
    ...
    def _apply_dark_theme(self):
        ...
        style.map("TButton", background=[("active", colors["button_active"]), ("pressed", colors["button_pressed"])])
        ...
        style.configure("TEntry", fieldbackground=colors["input_bg"], foreground=colors["fg"], insertcolor=colors["fg"])
        style.map(
            "TEntry",
        ...
        style.configure("Vertical.TScrollbar", background=colors["button"], troughcolor=colors["background"], arrowcolor=colors["fg"])
        style.configure("Horizontal.TScrollbar", background=colors["button"], troughcolor=colors["background"], arrowcolor=colors["fg"])
        style.configure("TProgressbar", background=colors["accent"], troughcolor=colors["background"])
        style.configure("Accent.Horizontal.TProgressbar", background=colors["accent"], troughcolor=colors["background"])
